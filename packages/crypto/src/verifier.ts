 ...
 */
function deriveVerifierTag(key: VaultKey): Buffer {
  const derived = hkdfSync('sha256', key.key, Buffer.alloc(0), VERIFIER_INFO, VERIFIER_TAG_BYTES);
...
export async function verify(masterPassword: string, verifier: VerifierRecord): Promise<boolean> {
  ...
  const candidateTag = deriveVerifierTag(key);
  const storedTag = Buffer.from(verifier.verifier, 'base64');

  if (candidateTag.length !== storedTag.length) {
    return false;
  }
  return timingSafeEqual(candidateTag, storedTag);
}
